...
Wrangler and CRE staging configs now contain the actual D1 UUID and planned origin `https://confidential-agent-jobs-staging.gilbertsahumada.workers.dev`. The staging config contains the verified receiver address; `writeReport` is now true; both HTTPS report dry runs passed. Both staging fixtures were subsequently broadcast with explicit authorization and verified on Arc. See `docs/evidence/https-arc-report-summary.json`. Resource creation commands below are the runbook; check existing resources before re-running them.
