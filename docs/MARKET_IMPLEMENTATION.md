...
Wallet connection is distinct from SIWE authentication. The selected EIP-6963/EIP-1193 provider is detected through a silent eth_accounts read; its address appears before any signature. Provider listeners attach before login and cover accountsChanged, chainChanged, connect and disconnect. Returning to the tab rechecks account/network without opening wallet prompts. Connect wallet automatically uses the remembered extension, MetaMask when available, or the first detected provider; no separate selector is shown. The provider address links to the explorer defined by the configured chain (currently Arc Testnet).
...
Wallet connection feedback distinguishes user rejection of connection, network switching and SIWE signing. Nested provider error codes are inspected without exposing raw extension errors; pending requests and missing wallets have actionable messages. Progress text explains the current step, and canceled requests release the connection lock for retry. Browser checks cover all three cancellation steps and pending requests on mobile and desktop.

Unanswered wallet requests can be abandoned with Cancel connection and time out after 60 seconds. Abandoning invalidates the login attempt and ignores late wallet responses; it cannot close a pending request inside the extension. Wallet transports do not automatically retry prompts. Browser regression tests exercise a provider that never settles, manual cancellation, timeout, late responses and retry.
