...
The provider service and scoped API passed local tests and were published to staging after explicit authorization to store PROVIDER_SERVICE_TOKEN. The same credential is retained in ignored .local/provider.env and apps/web/.dev.vars with permissions 0600. Worker version: ac7b2ce3-63c9-4674-91ce-71921c9cb788. HTTPS checks confirmed route authorization, no-store, denial of the provider credential on the private context route, and denial of anonymous Jobs access. The read-only service completed with zero correlated requests. Signing remains disabled; no service private key was loaded and no transaction was sent. Remote continuous operation and real funded journeys remain pending. See AUTONOMOUS_PROVIDER.md and evidence/autonomous-provider-staging.json.
