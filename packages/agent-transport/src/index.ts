...
export const sendSchema = z.strictObject({
  message: z.strictObject({
    ...
    parts: z
      .array(
        z.strictObject({
          data: z.strictObject({ probeId: probeIdSchema, input: inputSchema }),
  ...
  configuration: z
    .strictObject({
      ...
      acceptedOutputModes: z.array(z.literal('application/json')).optional(),
    })
    .optional(),
});
...
const taskSchema = z.object({
  id: probeIdSchema,
  contextId: probeIdSchema,
  status: z.object({ state: z.literal('TASK_STATE_COMPLETED') }),
  artifacts: z
    .array(
      z.object({
        artifactId: z.string(),
        parts: z
          .array(
            z.object({
              data: z.object({
                envelope: z.unknown(),
                resultHash: z.string().regex(/^0x[0-9a-f]{64}$/),
              }),
            }),
          )
          .length(1),
