...
async function main() {
  ...
  const configuredOrigin =
    process.env.MARKET_EXPECTED_ORIGIN ?? 'http://127.0.0.1:8787';
  ...
  if (anonymous.status !== 401)
    throw new Error('Private jobs are not protected');
  ...
  if (
    ...
    throw new Error('Cookie flags missing');
  const jobs = await request('/api/jobs', undefined, cookie);
  if (!jobs.ok || jobs.headers.get('cache-control') !== 'no-store')
    throw new Error('Jobs response failed');
  if (((await jobs.json()) as { jobs: unknown[] }).jobs.length !== 0)
    throw new Error('Unexpected jobs');
  const replay = await request('/api/auth/verify', {
    nonce: challenge.nonce,
    signature,
  });
  if (replay.status !== 401) throw new Error('Replay accepted');
  const wrongOrigin = await fetch(origin + '/api/auth/logout', {
    method: 'POST',
    headers: {
  ...
  if (wrongOrigin.status !== 403)
    throw new Error('Cross-origin write accepted');
