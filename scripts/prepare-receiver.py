...
artifact = json.loads(
    (root / "packages/contracts/out/ProbeReceiver.sol/ProbeReceiver.json").read_text()
)
    ...
    with urlopen(
        Request(
            ...
            ).encode(),
            {
                ...
                "User-Agent": "private-hire-probe/0.1",
            },
        ),
        timeout=30,
    ) as r:
...
summary = {
    "chainId": 5042002,
    "contract": "ProbeReceiver",
    "simulationOnly": True,
    "constructorMockForwarder": forwarder,
    "value": "0",
    "estimatedGas": gas,
    "gasPriceWei": str(price),
    "estimatedNativeUSDC": str(Decimal(gas * price) / Decimal(10**18)),
    "unsignedTransaction": ".local/receiver-deployment.json",
    "broadcast": False,
}
(root / "docs/evidence/receiver-deployment-review.json").write_text(
