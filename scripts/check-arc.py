...
code = {
    k: (len(rpc("eth_getCode", [v, "latest"])) - 2) // 2 for k, v in addresses.items()
}
assert code["forwarder"] > 0 and code["mockForwarder"] > 0
value = int(
    rpc("eth_call", [{"to": addresses["usdc"], "data": "0x313ce567"}, "latest"]), 16
)
assert value == 6
result = {
    ...
    "codeBytes": code,
    "usdcDecimals": value,
    "mode": "read-only RPC; no deployment or transaction",
}
